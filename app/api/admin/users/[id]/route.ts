...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Format response for both UserManagementPage and TransactionDetailPage
    const userData = {
      id: user._id.toString(),
      fullName: user.fullName || "Unknown",
      name: user.fullName || "Unknown", // Alias for UserManagementPage
      username: user.username || "N/A",
      email: user.email || "N/A",
      phone: user.phone || "N/A",
      phoneNumber: user.phone || "N/A", // Alias for UserManagementPage
      accountNumber: user.accountNumber || "N/A",
      balance: user.balance || 0,
      checkingAccountNumber: user.accountNumber || "N/A", // Alias for UserManagementPage
      checkingBalance: user.balance || 0, // Alias for UserManagementPage
